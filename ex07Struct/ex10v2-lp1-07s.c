#include <stdio.h>
#include <string.h>
#define JOGADORES 10

struct Ponto {
	int x;
	int y;
};

struct Personagem {
	short int id;
	struct Ponto pos;
	int pont;
	char visivel;
};

void lepnt (struct Ponto * pnt) {
	printf("Insira X: ");
	scanf("%d", &pnt->x);
	printf("Insira Y: ");
	scanf("%d", &pnt->y);
}

void lepers (struct Personagem * pers) {
	printf("Insira Id: ");
	scanf("%hu", &pers->id);
	lepnt(&pers->pos);
	pers->pont = 0;
	pers->visivel = 1;			// determina se o personagem deve aparecer no mapa
}

struct Ponto ptmax (struct Ponto pt1, struct Ponto pt2) {			// cria um ponto com os maiores x e y entre os dois pontos; usado para determinar o tamanho do mapa
	struct Ponto mai = {pt1.x, pt1.y};
	if (mai.x < pt2.x) {
		mai.x = pt2.x;
	}
	if (mai.y < pt2.y) {
		mai.y = pt2.y;
	}
	return mai;
}

void defmapa (struct Personagem pers[], int tamn, int xf, int yf, struct Personagem mapa[xf][yf]) {
	// coloca os personagens no mapa; pers é um vetor com todos os tamn personagens; mapa é um vetor bidimensional de dimensões xf por yf

	for (int i = 0; i < tamn; i++) {
		mapa[pers[i].pos.x][pers[i].pos.y] = pers[i];
	}
}

void desenhamapa(int xf, int yf, struct Personagem mapa[xf][yf]) {
	for (int i = 0; i < xf; i++) {
		printf("%3d ", i);				// indices verticais
	}
	printf("\n");

	for (int i = 0; i < yf; i++) {
		for (int j = 0; j < xf; j++) {
			if (!mapa[j][i].visivel) {
				printf("   ");
			}
			else {
				printf("%3hu", mapa[j][i].id);
			}
			printf(" ");
		}
		printf("%-3d", i);				// indices horizontais
		printf("\n");
	}
}

int main(void) {
	struct Personagem pers[JOGADORES];
	struct Ponto limite;
	for (int i = 0; i < JOGADORES; i++) {

		lepers(&pers[i]);
		if (i == 0) {
			limite = pers[i].pos;
		}
		else {
			limite = ptmax(limite, pers[i].pos);
		}
	}

	limite.x += 1;
	limite.y += 1;

	struct Personagem mapa[limite.x][limite.y];
	memset(mapa, 0, (limite.x*limite.y)*sizeof(struct Personagem));		// limpa o mapa
	defmapa(pers, JOGADORES, limite.x, limite.y, mapa);

	desenhamapa(limite.x, limite.y, mapa);

}
